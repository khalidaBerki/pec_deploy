...
const SelectCategorie: React.FC<SelectCategoriesProps> = ({
  ...
  useEffect(() => {
    if (selectedCategory) {
      setSelectedValue(selectedCategory.id)
  ...
  return (
    <div className="">
      <label className="mb-3 block text-body-sm font-medium text-dark dark:text-white">Sélectionner un Rayon</label>
      ...
      <div className="relative z-20 bg-transparent dark:bg-dark-2">
        ...
        >
          ...
          {categories.map((categorie) => (
            <option key={categorie.id} value={categorie.id} className="text-dark-6">
              {categorie.nom}
            </option>
          ))}
        </select>
